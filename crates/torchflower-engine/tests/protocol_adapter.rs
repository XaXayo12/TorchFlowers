use bedrock::protocol::{
    ...
    v662::{
        enums::{
            ComplexInventoryTransactionType, ConnectionFailReason, InputMode, NewInteractionModel,
...
use torchflower_engine::{
    auth::minecraft::MinecraftAuth,
    bedrock::{
        local_network::{
            codec::{decode_packets, encode_packets},
            encryption::Encryption,
        },
        protocol_adapter::BedrockProtocolAdapter,
    },
};
...
fn adapter_derives_bedrock_encryption_from_server_handshake() {
    ...
    let mut server_encryption = Encryption::new(&server_private, &client_public, &salt);
