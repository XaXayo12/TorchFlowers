...
async fn health() -> Json<serde_json::Value> {
    Json(json!({ "ok": true, "service": "torchflower-engine" }))
}

async fn metrics(State(state): State<AppState>) -> EngineResult<Json<crate::pool::MemoryStats>> {
    Ok(Json(state.bots.memory_stats().await))
}
...
async fn create_server(
    ...
    let server = servers
        ...
        .ok_or_else(|| {
            crate::error::EngineError::NotFound("server missing after insert".to_string())
        })?;
    Ok(Json(server))
}
...
async fn create_bot(
    ...
    let bot = bots.into_iter().find(|bot| bot.id == id).ok_or_else(|| {
        crate::error::EngineError::NotFound("bot missing after insert".to_string())
    })?;
    Ok(Json(bot))
}
