...
impl Reader<FragmentMeta> for FragmentMeta {
    fn read(buf: &mut binary_util::ByteReader) -> Result<FragmentMeta, std::io::Error> {
        let size = buf.read_u32()?;
        let id = buf.read_u16()?;
        let index = buf.read_u32()?;
        Ok(FragmentMeta { size, id, index })
    }
}

impl Writer for FragmentMeta {
...
impl Writer for FramePacket {
    fn write(&self, buf: &mut binary_util::ByteWriter) -> Result<(), std::io::Error> {
        // PrismarineJS/raknet-native sends Bedrock FrameSets with 0x80
        // (valid datagram, no extra datagram flags). DonutSMP ACKs 0x84, but
        // silently drops the fragmented Login before Bedrock login processing.
        buf.write_u8(0x80)?;
