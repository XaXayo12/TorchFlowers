...
pub(crate) fn record_range_values(start: u32, end: u32) -> RangeInclusive<u32> {
    start.min(end)..=start.max(end)
}

#[cfg(test)]
mod tests {
    use super::record_range_values;

    #[test]
    fn ack_range_records_are_inclusive() {
        assert_eq!(
            record_range_values(6, 25).collect::<Vec<_>>(),
            (6..=25).collect::<Vec<_>>()
        );
    }

    #[test]
    fn ack_range_records_tolerate_reversed_bounds() {
        assert_eq!(
            record_range_values(25, 6).collect::<Vec<_>>(),
            (6..=25).collect::<Vec<_>>()
        );
    }
}
