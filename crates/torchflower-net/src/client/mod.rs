...
//!
//! # Getting Started
//! Connecting to a server is extremely easy with `torchflower-net`, and can be done in a few lines of code.
...
fn frame_body_id(frame: &crate::protocol::frame::Frame) -> Option<u8> {
...
fn log_frame_packet(direction: &str, packet: &crate::protocol::frame::FramePacket) {
    raknet_trace!(
...
impl Client {
    ...
    /// # Example
    /// ```rust ignore
    /// use torchflower_net::client::Client;
    ///
    ...
    pub fn new(version: u8, mtu: u16) -> Self {
        let internal_recv_capacity = std::env::var("RAKNET_INTERNAL_RECV_BUFFER")
            .ok()
            .and_then(|value| value.trim().parse::<usize>().ok())
            .filter(|value| *value > 0)
            .unwrap_or(4096);
        let (internal_send, internal_recv) = bounded::<Vec<u8>>(internal_recv_capacity);
        Self {
    ...
    pub async fn connect<Addr: for<'a> Into<PossiblySocketAddr<'a>>>(
        ...
        let socket_task = task::spawn(async move {
            ...
            loop {
                ...
                select! {
                    killed = notifier.wait() => {
                        if killed {
                            eprintln!("[RAKNET_CLOSE] source=socket_task reason=close_notifier");
                            rakrs_debug!(true, "[CLIENT] Socket task closed");
                            break;
                        }
                    }
                    ...
                    recv = socket.recv(&mut buf) => {
                        match recv {
                            Ok(l) => length = l,
                            Err(e) => {
                                eprintln!("[RAKNET_SOCKET] event=recv_error error={}", e);
                                rakrs_debug!(true, "[CLIENT] Failed to receive packet: {}", e);
                                continue;
                            }
                        }
                        ...
                        // this allows the user to customize their own packet handling
                        if let Err(_) = net_send.send(buf[..length].to_vec()).await {
                            eprintln!("[RAKNET_CLOSE] source=socket_task reason=network_recv_channel_closed");
                            rakrs_debug!(true, "[CLIENT] Failed to send packet to network recv channel. Is the client closed?");
                            break;
                        }
                    }
            ...
            eprintln!("[RAKNET_CLOSE] source=socket_task reason=exit_loop");
        });
    ...
    fn init_recv_task(
        ...
        return Ok(task::spawn(async move {
            'task_loop: loop {
                ...
                macro_rules! recv_body {
                    ($pk_recv: expr) => {
                        ...
                        #[cfg(feature = "async_tokio")]
                        if let None = $pk_recv {
                            eprintln!("[RAKNET_CLOSE] source=recv_task reason=network_recv_channel_closed");
                            rakrs_debug!(true, "[CLIENT] (recv_task) Failed to receive anything on network channel, is there a sender?");
                            break 'task_loop;
                        }
                        ...
                        if *client_state == ConnectionState::Disconnecting {
                            eprintln!("[RAKNET_CLOSE] source=recv_task reason=client_state_disconnecting");
                        ...
                        let packet_bytes = $pk_recv.unwrap();
                        if packet_bytes.is_empty() {
                            eprintln!("[RAKNET_RECV_TASK] event=empty_network_packet");
                            continue;
                        }
                        let mut buffer = ByteReader::from(packet_bytes);

                        match buffer.as_slice()[0] {
                            0x80..=0x8d => {
                                if let Ok(frame_packet) = FramePacket::read(&mut buffer) {
                                    log_frame_packet("rx", &frame_packet);
                                    ...
                                    'buf_loop: for pk_buf_raw in buffers {
                                        ...
                                        if let Ok(rak_packet) = RakPacket::read(&mut pk_buf) {
                                            match rak_packet {
                                                RakPacket::Online(pk) => {
                                                    match pk {
                                                        ...
                                                        OnlinePacket::Disconnect(_) => {
                                                            eprintln!("[RAKNET_CLOSE] source=recv_task reason=online_disconnect_packet");
                                                            rakrs_debug!(
                                                                true,
                                                ...
                                                RakPacket::Offline(_) => {
                                                    eprintln!("[RAKNET_RECV_TASK] event=offline_packet_after_handshake");
                                            ...
                                            // we send this packet
                                            if let Err(_) = internal_sender.send(pk_buf_raw).await {
                                                eprintln!("[RAKNET_CLOSE] source=recv_task reason=internal_recv_channel_closed");
                                                rakrs_debug!(true, "[CLIENT] Failed to send packet to internal recv channel. Is the client closed?");
                                                break 'task_loop;
                                            }
                                        }
                                    ...
                                    eprintln!("[RAKNET_RECV_TASK] event=frame_packet_parse_error first_byte={:#04x} len={}", buffer.as_slice()[0], buffer.as_slice().len());
                                }
                            }
                            NACK => {
                                if let Ok(nack) = Ack::read(&mut buffer) {
                                    raknet_trace!("[RR_PACKET_LOG] direction=rx stage=ack packet=NACK records={:?}", nack.records);
                                    let mut send_q = send_queue.write().await;
                                    let to_resend = send_q.nack(nack);

                                    if to_resend.len() > 0 {
                                        for ack_packet in to_resend {
                                            send_q.resend_frame_packet(&ack_packet).await;
                                        }
                                    }
                                } else {
                                    eprintln!("[RAKNET_NACK] event=parse_error len={}", buffer.as_slice().len());
                                }
                            }
                            ACK => {
                                if let Ok(ack) = Ack::read(&mut buffer) {
                                    raknet_trace!("[RR_PACKET_LOG] direction=rx stage=ack packet=ACK records={:?}", ack.records);
                                    let mut send_q = send_queue.write().await;
                                    raknet_trace!("[RAKNET_SEND_STATE] event=ack_before {}", send_q.debug_state());
                                    send_q.ack(ack.clone());
                                    raknet_trace!("[RAKNET_SEND_STATE] event=ack_after {}", send_q.debug_state());
                                } else {
                                    eprintln!("[RAKNET_ACK] event=parse_error len={}", buffer.as_slice().len());
                                }
                            }
                            _ => {
                                ...
                                if let Err(_) = internal_sender.send(buffer.as_slice().to_vec()).await {
                                    eprintln!("[RAKNET_CLOSE] source=recv_task reason=internal_recv_channel_closed_unknown_packet");
                                    rakrs_debug!(true, "[CLIENT] Failed to send packet to internal recv channel. Is the client closed?");
                                    break 'task_loop;
                                }
                            }
                ...
                select! {
                    killed = closed_dispatch.wait() => {
                        if killed {
                            eprintln!("[RAKNET_CLOSE] source=recv_task reason=close_notifier");
                            rakrs_debug!(true, "[CLIENT] Recv task closed");
                            break 'task_loop;
                        }
                    }
            ...
            eprintln!("[RAKNET_CLOSE] source=recv_task reason=exit_loop");
        }));
    }
                ...
                macro_rules! tick_body {
                    () => {
...

                        {
                            ...
                            if (recv + (timeout_time / 2)) <= current_epoch()
                                && state.is_reliable()
                            {
                                *state = ConnectionState::TimingOut;
                                ...
                                rakrs_debug!(
                                    true,
                                    "[RAKNET_KEEPALIVE] event=send_connected_ping reason=timeout_probe reliability=Unreliable"
                                );
                                if let Ok(_) = send_q
                                    .send_packet(ping.into(), Reliability::Unreliable, true)
                                    .await
                                {}
                            }
                            ...
                            if last_ping >= 500 {
                                ...
                                rakrs_debug!(
                                    true,
                                    "[RAKNET_KEEPALIVE] event=send_connected_ping reason=periodic reliability=Unreliable"
                                );
                                if let Ok(_) = send_q
                                    .send_packet(ping.into(), Reliability::Unreliable, true)
                                    .await
                        ...
                        let (ack, nack, recv_debug_state) = {
                            let mut recv_q = recv_queue.lock().await;
                            let ack = Ack::from_records(recv_q.ack_flush(), false);
                            let nack = Ack::from_records(recv_q.nack_queue(), true);
                            let debug_state = recv_q.debug_state();
                            (ack, nack, debug_state)
                        };
                        raknet_trace!("[RAKNET_RECV_STATE] event=tick {}", recv_debug_state);

                        if ack.records.len() > 0 || nack.records.len() > 0 {
                            let mut send_q = send_queue.write().await;
                            // Flush the queue of acks and nacks, and respond to them.
                            if ack.records.len() > 0 {
                                raknet_trace!(
                                    "[RAKNET_ACK] direction=tx records={:?}",
                                    ack.records
                                );
                                if let Ok(p) = ack.write_to_bytes() {
                                    send_q.send_stream(p.as_slice()).await;
                            ...
                            if nack.records.len() > 0 {
                                raknet_trace!(
                                    "[RAKNET_NACK] direction=tx records={:?}",
                                    nack.records
                                );
                                if let Ok(p) = nack.write_to_bytes() {
                                    send_q.send_stream(p.as_slice()).await;
