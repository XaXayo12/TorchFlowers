...
macro_rules! expect_reply {
    ($socket: expr, $reply: ty, $timeout: expr) => {{
        ...
        loop {
            ...
            // (outer ids 0x80–0x8d). Unwrap the first frame body so that the
        ...
        task::spawn(async move {
            ...
            .await
            {
                DiscoveryStatus::Discovered(m, c) => {
                    rakrs_debug!(true, "[CLIENT] Discovered MTU size: {}, cookie: {:#x}", m, c);
                    mtu = m;
                    cookie = c;
                }
                    ...
                    0x80..=0x8d => {
                        if let Ok(pk) = FramePacket::read(&mut reader) {
                            ...
                            if let Ok(buf) = ack.write_to_bytes() {
                                if let Err(e) =
                                    socket.send_to(buf.as_slice(), socket.peer_addr().unwrap()).await
                                {
                                    rakrs_debug!(true, "[CLIENT] Failed to send ACK during handshake: {:?}", e);
                                }
                            }
