...
impl Reader<Motd> for Motd {
    fn read(buf: &mut ByteReader) -> Result<Motd, std::io::Error> {
        ...
        let motd = String::from_utf8(str_buf).map_err(|err| {
            std::io::Error::new(
                std::io::ErrorKind::InvalidData,
                format!("Invalid motd UTF-8: {err}"),
            )
        })?;
        let parts = motd
            .split(";")
        ...
        Ok(Motd {
            edition,
            name,
            protocol: protocol.trim().parse().unwrap_or(0),
            version,
            player_count: player_count.trim().parse().unwrap_or(0),
            player_max: player_max.trim().parse().unwrap_or(0),
            server_guid: server_guid.trim().parse().unwrap_or(0),
            sub_name,
